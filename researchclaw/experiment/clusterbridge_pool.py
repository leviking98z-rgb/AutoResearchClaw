...
class ClusterBridgePool:
    ...
    def prepare_cache(self) -> dict[str, BridgeResult]:
        ...
        if not cache_dir or not archive:
            return {}
        digest_file = f"{archive}.sha256"
        command = (
            "set -euo pipefail; "
            f"archive={shlex.quote(archive)}; "
            f"digest_file={shlex.quote(digest_file)}; "
            f"target={shlex.quote(cache_dir)}; "
            "[ -s \"$archive\" ]; "
            "parent=$(dirname \"$target\"); base=$(basename \"$target\"); "
            "mkdir -p \"$parent\"; "
            "lock=\"$parent/.${base}.prepare.lock\"; "
            "exec 9>\"$lock\"; flock 9; "
            "if [ -s \"$digest_file\" ]; then "
            "digest=$(awk 'NF {print $1; exit}' \"$digest_file\"); "
            "else digest=$(sha256sum \"$archive\" | awk '{print $1}'); fi; "
            "case \"$digest\" in "
            "????????????????????????????????????????????????????????????????) ;; "
            "*) printf 'invalid cache digest: %s\\n' \"$digest\" >&2; exit 2;; "
            "esac; "
