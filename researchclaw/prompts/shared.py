...
_DEFAULT_SUB_PROMPTS: dict[str, dict[str, Any]] = {
    ...
    "architecture_planning": {
        ...
        "user": (
            ...
            "## Scientific Authenticity (FAIL CLOSED)\n"
            "- The blueprint MUST execute the exact models, datasets, tasks, and "
            "measurements declared in the experiment plan.\n"
            "- NEVER replace a real model/benchmark experiment with a synthetic DGP, "
            "simulated trajectories, mock inference, random correctness/calibration, "
            "toy utility functions, or placeholder metrics.\n"
            "- A cheap pilot means fewer real examples/iterations or a smaller real "
            "checkpoint — it does NOT mean simulated evidence.\n"
            "- If a declared asset is unavailable, design an explicit availability "
            "check that exits non-zero and reports the missing asset; never silently "
    ...
    "generate_single_file": {
        ...
        "user": (
            ...
            "12. SCIENTIFIC AUTHENTICITY: If the plan declares a real model, dataset, "
            "benchmark, or API-backed subject, this file MUST participate in loading "
            "or executing that real asset. NEVER implement synthetic DGPs, simulated "
            "LLM trajectories, mock inference, random correctness/calibration/reward, "
            "or comments that say a production experiment will replace this code.\n"
            "13. FAIL CLOSED: Missing model weights, datasets, licenses, credentials, "
            "or cache entries must produce a clear non-zero failure. Do not silently "
            "fall back to synthetic/random data or placeholder metrics.\n\n"
    ...
    "code_reviewer": {
        ...
        "user": (
            ...
            "7. **SCIENTIFIC AUTHENTICITY**: Verify that declared real models, "
            "datasets, benchmarks, and APIs are actually loaded/executed. Treat "
            "synthetic DGPs, simulated trajectories/inference, random scientific "
            "outcomes, toy proxies, placeholder metrics, or 'production will replace "
            "this' comments as CRITICAL whenever the plan requires real evidence.\n"
            "8. **FAIL-CLOSED ASSETS**: Missing real assets must cause an explicit "
            "non-zero failure, never a silent synthetic/random fallback.\n\n"
            "## Output Format (JSON)\n"
            "```json\n"
            '{{\n'
